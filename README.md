##Olá! Me chamo João Gabriel  👋

## Sobre mim:
<div>
  <p>
   Sou estudante de Sistemas de Informação e Técnico em Informática, apaixonado por tecnologia e desenvolvimento de sistemas. Tenho experiência com suporte técnico, implantação de sistemas e atendimento ao cliente. Atualmente, estou aprimorando meus conhecimentos em desenvolvimento Web, utilizando tecnologias como HTML, CSS, JavaScript e Node.js, além de estudar banco de dados e Git. Busco evoluir constantemente e desenvolver soluções práticas e eficientes.
  </p>
</div>

## 👨🏽‍💻🚀 Minhas Tecnologias 

<div style="display: inline_block">
  <img align="center" alt="HTML" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg">
  <img align="center" alt="CSS" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg">
  <img align="center" alt="JavaScript" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-plain.svg">
  <img align="center" alt="Python" height="30" width="40" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg">
  <img align="center" alt="Node.js" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nodejs/nodejs-original.svg" />
  <img align="center" alt="Django" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/django/django-plain.svg" />               
  <img align="center" alt="SQLITE" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/sqlite/sqlite-original.svg" />
  <img align="center" alt="Postgresql" height="30" width="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/postgresql/postgresql-original.svg" />
            
</div><br>

## 🌐 Minhas Redes Sociais
  
<div> 
  <a href="[https://www.linkedin.com/in/ivisconfessor](https://www.linkedin.com/in/jo%C3%A3o-gabriel-eug%C3%AAnio-soares-2a6319265/)" target="_blank"><img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a> 
</div>
